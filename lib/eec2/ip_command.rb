...
class IpCommand < SubCommand
  ...
  def remove(args)
    ...
    instance_infos[0][:net_interfaces].each do |iface|
      # noinspection RubyResolve
      remove_from_this_iface = iface.private_ip_addresses.each & args
      # noinspection RubyResolve
    ...
    $stderr.puts "Note: the following addresses were not found in instance #{instance_name}: #{args}" unless args.empty?
  end
      ...
      i[:net_interfaces].each do |iface|
        iface.private_ip_addresses.each do |addr|
          if @sub_options[:long]
            ...
            puts "#{i[:name]} #{addr.private_ip_address}"
          end
        end
      end
    end
